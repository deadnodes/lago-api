...
- `LAGO_POSTER_ENABLED` — when true, finalized invoice emails use the DeadNodes Poster service instead of Lago SMTP. Example: `LAGO_POSTER_ENABLED=true`
- `LAGO_POSTER_API_URL` — base URL of the Poster service. Example: `LAGO_POSTER_API_URL=https://poster.example.internal`
- `LAGO_POSTER_API_TOKEN` — bearer token used for Poster API writes. Example: `LAGO_POSTER_API_TOKEN=***`
- `LAGO_POSTER_OPEN_TIMEOUT` — Poster connection timeout in seconds. Defaults to `5`. Example: `LAGO_POSTER_OPEN_TIMEOUT=5`
- `LAGO_POSTER_READ_TIMEOUT` — Poster response timeout in seconds. Defaults to `30`. Example: `LAGO_POSTER_READ_TIMEOUT=30`
